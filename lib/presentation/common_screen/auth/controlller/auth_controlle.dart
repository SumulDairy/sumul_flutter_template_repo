          ...
          if ((sendResponse.status ?? false)) {
            if (sendResponse.data != null) {
              ...
              AppServices.settings.empNo =
                  sendResponse.data?.custcd ?? usernameController.value.text;
              Get.focusScope?.unfocus();
              AppServices.settings.mobileNo =
                  sendResponse.data?.mobileno ?? usernameController.value.text;
              AppServices.settings.isDistributor =
                  sendResponse.data?.isdistributor ?? false;
              AppServices.settings.empNo = sendResponseData.custcd ?? "";
              AppServices.settings.name = sendResponseData.custname ?? "";

              Get.toNamed(Routes.OTP_SCREEN, arguments: sendResponse.data);
            } else {
              await Get.dialog(
                ShowAleartComman(
